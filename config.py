...
class GSMambaConfig:
    ...
    # Phase 3 (flag-gated, default OFF): predict per-Gaussian 3D motion (velocity +
    # angular velocity, plus acceleration if motion_accel) and forward-warp each frame's
    # Gaussians to the query time t (advect positions, rotate orientations), then merge
    # the warped clouds weighted by temporal proximity (rasterizer composites by depth).
    ...
    motion_temporal_tau: float = 1.0   # temporal opacity-weight softness (x avg frame gap); <=0 -> uniform
    refine_real_coverage: bool = False # feed refine a real per-pixel coverage map instead of the broadcast-mean opacity
...
def update_config_from_args(config: FullConfig, args) -> FullConfig:
    ...
    if getattr(args, 'x4k_crop_size', None) is not None:
        config.data.x4k_crop_size = int(args.x4k_crop_size)
